...
namespace NanoXLSX.Demo.UseCases
{
    ...
    public class Read
    {
        ...
        {
            // First, ensure the file exists by running BasicDemo
            Console.WriteLine("Creating sample file for reading...");

            Workbook wb = WorkbookReader.Load(BASIC_DEMO_FILE);                                 // Load the workbook 'basic.xlsx' from file 
            ...
            {
                Workbook wb2 = WorkbookReader.Load(fs);                                         // Read the file stream
